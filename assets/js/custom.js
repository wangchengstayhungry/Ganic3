...
function showData(mode,url,form_action='#'){  
  rowData = table.row('.selected').data(); 
    if(rowData){
        ...
        if(mode=="quotation_view" || mode=="quotation_edit"){
        ...
        if(mode=="invoice_view" || mode=="invoice_edit" || mode == "open_edit" || mode == "open_stock_edit"){
        ...
        if(mode=="receipt_view" || mode=="receipt_edit"){
          window.location.href=url+rowID;
          $("#form_data").html(''); // remove content of form.
          $("#refresh").click();//refresh  the datatable.
          $("#list_table").show(); // show data table
            ...
            $.confirm({
                ...
                confirm: function(button) {
                    $.post(url,{ rowID: rowID } , function(result){
                        ...
                        $("#refresh").click();//refresh  the datatable.
                        $("#list_table").show(); // show data table
                        $("#message_area").html("<div class='alert alert-success fade in'><button type='button' class='close close-sm' data-dismiss='alert'><i class='fa fa-times'></i></button>Record Deleted Succesfully!</div>");
                    });
                    showHideButtons("list");
                },
                cancelButton: 'No',
