<?php
	...
	class Quotation extends MY_Controller {
		...
		public function quotation_setting($action="form")
		{
			...
			if($action=="form"){
				$types=array("order_entry"=> "Order Entry","deleivered_order"=> "Deleivered Order","invoice"=>"Invoice");
				if(!is_null($quotation_details)){
					$this->body_vars['quotation_type_options']=createSimpleDropdown($types,"Quotation Type",$quotation_details->quotation_type,0);
				}
				else{
					$this->body_vars['quotation_type_options']=createSimpleDropdown($types,"Quotation Type",'',0);	
				}
		...
		public function index()
		{
			...
			/*==========================================*/
			$this->body_vars['salesman_options']=$this->custom->createDropdownSelect("salesman_master",array('s_id','s_name'),"Sales Person");
			/*==========================================*/
		...
		public function create_new_quotation($action="new")
		{
			...
			if($post){
				if(count($post['product_id'])>=1){
					...
					redirect('quotation/quotationlist/pending');
				}
			}
		}
